...
<tr><td><b>Id</b></td><td><CopyableCode code="firebase.apphosting.operations" /></td></tr>
</tbody></table>
...
metadata,
response
FROM firebase.apphosting.operations
...
```sql
DELETE FROM firebase.apphosting.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC firebase.apphosting.operations.cancel 
