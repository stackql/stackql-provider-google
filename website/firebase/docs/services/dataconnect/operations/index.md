...
<tr><td><b>Id</b></td><td><CopyableCode code="firebase.dataconnect.operations" /></td></tr>
</tbody></table>
...
metadata,
response
FROM firebase.dataconnect.operations
...
```sql
DELETE FROM firebase.dataconnect.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC firebase.dataconnect.operations.cancel 
