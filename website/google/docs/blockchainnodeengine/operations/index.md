...
<tr><td><b>Id</b></td><td><CopyableCode code="google.blockchainnodeengine.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.blockchainnodeengine.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.blockchainnodeengine.operations.cancel 
