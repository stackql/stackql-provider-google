...
<tr><td><b>Id</b></td><td><CopyableCode code="google.networkconnectivity.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.networkconnectivity.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.networkconnectivity.operations.cancel 
