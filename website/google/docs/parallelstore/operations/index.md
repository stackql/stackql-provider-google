...
<tr><td><b>Id</b></td><td><CopyableCode code="google.parallelstore.operations" /></td></tr>
</tbody></table>
...
metadata,
response
FROM google.parallelstore.operations
...
```sql
DELETE FROM google.parallelstore.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.parallelstore.operations.cancel 
