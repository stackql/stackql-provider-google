...
<tr><td><b>Id</b></td><td><CopyableCode code="google.datalineage.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.datalineage.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.datalineage.operations.cancel 
