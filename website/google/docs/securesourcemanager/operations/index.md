...
<tr><td><b>Id</b></td><td><CopyableCode code="google.securesourcemanager.operations" /></td></tr>
</tbody></table>
...
metadata,
response
FROM google.securesourcemanager.operations
...
```sql
DELETE FROM google.securesourcemanager.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.securesourcemanager.operations.cancel 
