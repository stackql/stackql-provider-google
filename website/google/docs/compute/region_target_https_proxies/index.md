...
Creates, updates, deletes, gets or lists a <code>region_target_https_proxies</code> resource.
...
- name: region_target_https_proxies
  props:
    - name: project
      value: string
      description: Required parameter for the region_target_https_proxies resource.
    - name: region
      value: string
      description: Required parameter for the region_target_https_proxies resource.
    - name: kind
...
```sql
DELETE FROM google.compute.region_target_https_proxies
WHERE project = '{{ project }}' --required
AND region = '{{ region }}' --required
AND targetHttpsProxy = '{{ targetHttpsProxy }}' --required
AND requestId = '{{ requestId }}';
...
```sql
EXEC google.compute.region_target_https_proxies.set_url_map 
@project='{{ project }}' --required, 
...
```sql
EXEC google.compute.region_target_https_proxies.set_ssl_certificates 
@project='{{ project }}' --required, 
@region='{{ region }}' --required, 
@targetHttpsProxy='{{ targetHttpsProxy }}' --required, 
@requestId='{{ requestId }}' 
