...
<tr><td><b>Id</b></td><td><CopyableCode code="google.clouddeploy.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.clouddeploy.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.clouddeploy.operations.cancel 
