...
<tr><td><b>Id</b></td><td><CopyableCode code="google.datafusion.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.datafusion.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.datafusion.operations.cancel 
