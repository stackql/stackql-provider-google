...
<tr><td><b>Id</b></td><td><CopyableCode code="google.vmmigration.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.vmmigration.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.vmmigration.operations.cancel 
