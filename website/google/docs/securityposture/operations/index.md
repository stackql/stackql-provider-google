...
<tr><td><b>Id</b></td><td><CopyableCode code="google.securityposture.operations" /></td></tr>
</tbody></table>
...
metadata,
response
FROM google.securityposture.operations
...
```sql
DELETE FROM google.securityposture.operations
WHERE organizationsId = '{{ organizationsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.securityposture.operations.cancel 
