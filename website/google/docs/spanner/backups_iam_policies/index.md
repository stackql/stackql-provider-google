...
<tr><td><b>Id</b></td><td><CopyableCode code="google.spanner.backups_iam_policies" /></td></tr>
</tbody></table>
...
<Tabs
    defaultValue="projects_instances_backups_get_iam_policy"
    values={[
        { label: 'projects_instances_backups_get_iam_policy', value: 'projects_instances_backups_get_iam_policy' }
    ]}
>
...
<tbody>
<tr>
    <td><a href="#projects_instances_backups_get_iam_policy"><CopyableCode code="projects_instances_backups_get_iam_policy" /></a></td>
...
<tr>
    <td><a href="#projects_instances_backups_set_iam_policy"><CopyableCode code="projects_instances_backups_set_iam_policy" /></a></td>
...
<tr>
    <td><a href="#projects_instances_backups_test_iam_permissions"><CopyableCode code="projects_instances_backups_test_iam_permissions" /></a></td>
...
<Tabs
    defaultValue="projects_instances_backups_set_iam_policy"
    values={[
        { label: 'projects_instances_backups_set_iam_policy', value: 'projects_instances_backups_set_iam_policy' }
    ]}
>
<TabItem value="projects_instances_backups_set_iam_policy">

Sets the access control policy on a database or backup resource. Replaces any existing policy. Authorization requires `spanner.databases.setIamPolicy` permission on resource. For backups, authorization requires `spanner.backups.setIamPolicy` permission on resource. For backup schedules, authorization requires `spanner.backupSchedules.setIamPolicy` permission on resource.

```sql
REPLACE google.spanner.backups_iam_policies
SET 
data__policy = '{{ policy }}'
WHERE 
...
<Tabs
    defaultValue="projects_instances_backups_test_iam_permissions"
    values={[
        { label: 'projects_instances_backups_test_iam_permissions', value: 'projects_instances_backups_test_iam_permissions' }
    ]}
>
<TabItem value="projects_instances_backups_test_iam_permissions">

Returns permissions that the caller has on the specified database or backup resource. Attempting this RPC on a non-existent Cloud Spanner database will result in a NOT_FOUND error if the user has `spanner.databases.list` permission on the containing Cloud Spanner instance. Otherwise returns an empty set of permissions. Calling this method on a backup that does not exist will result in a NOT_FOUND error if the user has `spanner.backups.list` permission on the containing instance. Calling this method on a backup schedule that does not exist will result in a NOT_FOUND error if the user has `spanner.backupSchedules.list` permission on the containing database.

```sql
EXEC google.spanner.backups_iam_policies.projects_instances_backups_test_iam_permissions 
@projectsId='{{ projectsId }}' --required, 
