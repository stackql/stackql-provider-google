...
<tr><td><b>Id</b></td><td><CopyableCode code="google.netapp.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.netapp.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.netapp.operations.cancel 
