...
<tr><td><b>Id</b></td><td><CopyableCode code="google.observability.operations" /></td></tr>
</tbody></table>
...
metadata,
response
FROM google.observability.operations
...
```sql
DELETE FROM google.observability.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.observability.operations.cancel 
