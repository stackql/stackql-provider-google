...
<tr><td><b>Id</b></td><td><CopyableCode code="google.datacatalog.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.datacatalog.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.datacatalog.operations.projects_locations_operations_cancel 
