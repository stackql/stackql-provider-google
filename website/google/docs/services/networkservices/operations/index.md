...
<tr><td><b>Id</b></td><td><CopyableCode code="google.networkservices.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.networkservices.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.networkservices.operations.cancel 
