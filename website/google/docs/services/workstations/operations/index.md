...
<tr><td><b>Id</b></td><td><CopyableCode code="google.workstations.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.workstations.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.workstations.operations.cancel 
