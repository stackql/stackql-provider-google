...
<tr><td><b>Id</b></td><td><CopyableCode code="google.developerconnect.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.developerconnect.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.developerconnect.operations.cancel 
