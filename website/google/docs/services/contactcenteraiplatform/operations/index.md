...
<tr><td><b>Id</b></td><td><CopyableCode code="google.contactcenteraiplatform.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.contactcenteraiplatform.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.contactcenteraiplatform.operations.cancel 
