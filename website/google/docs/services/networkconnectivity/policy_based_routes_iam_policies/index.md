...
Creates, updates, deletes, gets or lists a <code>policy_based_routes_iam_policies</code> resource.
...
```sql
EXEC google.networkconnectivity.policy_based_routes_iam_policies.test_iam_permissions 
@projectsId='{{ projectsId }}' --required, 
@policyBasedRoutesId='{{ policyBasedRoutesId }}' --required 
@@json=
'{
