...
<tr><td><b>Id</b></td><td><CopyableCode code="google.datastream.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.datastream.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.datastream.operations.cancel 
