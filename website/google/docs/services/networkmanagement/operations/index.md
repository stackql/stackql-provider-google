...
<tr><td><b>Id</b></td><td><CopyableCode code="google.networkmanagement.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.networkmanagement.operations
WHERE organizationsId = '{{ organizationsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.networkmanagement.operations.cancel 
