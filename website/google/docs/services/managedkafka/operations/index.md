...
<tr><td><b>Id</b></td><td><CopyableCode code="google.managedkafka.operations" /></td></tr>
</tbody></table>
...
metadata,
response
FROM google.managedkafka.operations
...
```sql
DELETE FROM google.managedkafka.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.managedkafka.operations.cancel 
