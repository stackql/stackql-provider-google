...
<tr><td><b>Id</b></td><td><CopyableCode code="google.oracledatabase.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.oracledatabase.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.oracledatabase.operations.cancel 
