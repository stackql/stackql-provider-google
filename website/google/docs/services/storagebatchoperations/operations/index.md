...
<tr><td><b>Id</b></td><td><CopyableCode code="google.storagebatchoperations.operations" /></td></tr>
</tbody></table>
...
metadata,
response
FROM google.storagebatchoperations.operations
...
```sql
DELETE FROM google.storagebatchoperations.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.storagebatchoperations.operations.cancel 
