...
<tr><td><b>Id</b></td><td><CopyableCode code="google.workloadmanager.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.workloadmanager.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.workloadmanager.operations.cancel 
