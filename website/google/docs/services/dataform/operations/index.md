...
<tr><td><b>Id</b></td><td><CopyableCode code="google.dataform.operations" /></td></tr>
</tbody></table>
...
metadata,
response
FROM google.dataform.operations
...
```sql
DELETE FROM google.dataform.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.dataform.operations.cancel 
