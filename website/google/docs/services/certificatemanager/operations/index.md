...
<tr><td><b>Id</b></td><td><CopyableCode code="google.certificatemanager.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.certificatemanager.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required;
```
...
```sql
EXEC google.certificatemanager.operations.cancel 
